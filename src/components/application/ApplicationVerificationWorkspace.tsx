...
function BankingPanel({ profileId, sourceKind, sourceId, state, banks, loading, onRefresh }: {
  ...
  useEffect(() => {
    if ((banks?.manual_statement_file_count ?? 0) > 0 && !(banks?.items ?? []).some((item) => item.status !== "removed")) {
      setBankTab("statements");
    }
  }, [banks?.items, banks?.manual_statement_file_count]);
  ...
  const evidenceSummary = banks?.evidence_summary;
  const manualMonths = evidenceSummary?.bank_statement_months ?? banks?.manual_statement_months ?? [];
  const requiredMonths = evidenceSummary?.bank_statement_required_months ?? 6;
  const manualFileCount = evidenceSummary?.bank_statement_file_count ?? banks?.manual_statement_file_count ?? 0;
  const manualAcceptedCount = evidenceSummary?.bank_statement_accepted_count ?? banks?.manual_statement_accepted_count ?? 0;
  const manualPendingCount = evidenceSummary?.bank_statement_processing_count ?? banks?.manual_statement_pending_count ?? 0;
  const manualRejectedCount = evidenceSummary
  ...
  return <><section className="verification-section bank-evidence-workspace">
    <header className="verification-section-head">
      <div><span className="lbl">Step 4</span><h3>Business bank evidence</h3><Sub>AI-accepted uploaded statements and client-authorized Plaid data are alternative evidence sources. Staff only override exceptions.</Sub></div>
    ...
    {needsClientBankRequest ? <div className="bank-request-toolbar">
      <div><b>{manualMonths.length ? `Request ${Math.max(0, requiredMonths - manualMonths.length)} missing statement month${requiredMonths - manualMonths.length === 1 ? "" : "s"}` : "Client bank request"}</b><Sub>{hasManualEvidence ? "Uploaded statements already count. Request only the missing periods; Plaid remains optional." : "Email the secure room so the client can upload statements or authorize Plaid. The PIN is shared separately."}</Sub></div>
    ...
    </div> : hasManualEvidence ? <Callout tone="ok" icon={<Icon name="check" size={16} />}>Uploaded statements provide six accepted months. Business banking is complete and the client does not need to connect Plaid.</Callout> : null}
    ...
    {(needsClientBankRequest || bankDeliveries.length) ? <div className="bank-delivery-strip" aria-live="polite">
