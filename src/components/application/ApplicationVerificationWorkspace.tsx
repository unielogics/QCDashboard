...
function BankingPanel({ profileId, sourceKind, sourceId, state, banks, loading, onRefresh }: {
  ...
  const manualFileCount = banks?.manual_statement_file_count ?? 0;
  const manualPendingCount = banks?.manual_statement_pending_count ?? 0;
  const hasManualEvidence = manualFileCount > 0;
  const needsClientBankRequest = !state?.business_banking_complete && !hasManualEvidence;
  ...
  return <section className="verification-section bank-evidence-workspace">
    ...
    </div> : hasManualEvidence && !banks?.manual_override ? <Callout tone="acc" icon={<Icon name="file" size={16} />}>{manualFileCount} uploaded bank statement file{manualFileCount === 1 ? " is" : "s are"} already available from Evidence. Review and approve the statement evidence below; the client does not need to connect Plaid.</Callout> : null}
