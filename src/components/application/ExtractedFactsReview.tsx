...
import {
  ...
  type ExtractedFactSuggestionGroup,
...
function suggestionDetail(group: ExtractedFactSuggestionGroup) {
  const fact = group.fact;
  return [
    fact.confidence != null
      ? `${Math.round(fact.confidence * 100)}% confidence`
      : "Confidence not reported",
    fact.extraction_method.replaceAll("_", " "),
    group.matchingValueCount > 1
      ? `confirmed in ${group.matchingValueCount} files`
      : null,
    group.alternativeValueCount
      ? `${group.alternativeValueCount} other value${group.alternativeValueCount === 1 ? "" : "s"} available`
      : null,
  ].filter(Boolean).join(" | ");
}
...
export function ExtractedFactsReview({ sourceKind, sourceId }: { sourceKind: ApplicationSourceKind; sourceId: string }) {
  ...
  const review = useMutation({ mutationFn: ({ id, action }: { id: string; action: "accept" | "reject" }) => apiCall<ExtractedFact>(`/application-profiles/${profileId}/extracted-facts/${id}/review`, { method: "POST", body: JSON.stringify({ action }) }), onSuccess: async () => {
    await Promise.all([
      qc.invalidateQueries({ queryKey: ["application-extracted-facts", profileId] }),
      qc.invalidateQueries({ queryKey: ["application-draft-status", profileId] }),
      qc.invalidateQueries({ queryKey: ["application-evidence-workspace", profileId] }),
      qc.invalidateQueries({ queryKey: ["application-intelligence", profileId] }),
  ...
  const suggestionGroups = groupSuggestedExtractedFacts(facts.data ?? []);
  if (facts.isLoading || !profile.data) return null;

  return <section className="extracted-facts-review">
    <header><div><span className="lbl">Extracted data review</span><h3>{suggestionGroups.length ? `${suggestionGroups.length} suggested field${suggestionGroups.length === 1 ? "" : "s"}` : "Evidence extraction reviewed"}</h3></div>{suggestionGroups.length ? <CellChip tone="warn">Review required</CellChip> : profile.data.is_draft ? <CellChip tone={draftStatus.data?.can_finalize ? "ok" : "warn"}>Draft</CellChip> : <CellChip tone="ok">Finalized</CellChip>}</header>
