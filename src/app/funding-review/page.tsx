...
const initialContact = {
  ...
  transaction_type: "",
  requested_amount: "",
  estimated_value_or_purchase_price: "",
  monthly_rent: "",
...
export default function DealerAIUnderwriterPage() {
  ...
  async function startIntake() {
    ...
    setStatus("");
    try {
      const payload = await call<IntakeResponse>("/public/funding-review/start", {
        method: "POST",
        body: JSON.stringify({
          ...
          requested_amount: numericOrNull(contact.requested_amount),
          estimated_value_or_purchase_price: numericOrNull(contact.estimated_value_or_purchase_price),
          monthly_rent: numericOrNull(contact.monthly_rent),
          estimated_credit_tier: contact.estimated_credit_tier.trim() || null,
          terms_accepted: true,
      ...
      if (typeof window !== "undefined") {
        window.history.replaceState(null, "", "/funding-review");
      }
    } catch (error) {
  ...
  async function requestDealerCode() {
    ...
    setStatus("");
    try {
      const payload = await call<{ ok: boolean; login_required?: boolean; message: string }>("/public/funding-review/login/start", {
        method: "POST",
        body: JSON.stringify({ email }),
      });
      setLoginCodeSent(Boolean(payload.login_required ?? true));
      setStatus(payload.message || "If a secure funding review exists for this email, a code has been sent.");
    } catch (error) {
  ...
  async function checkDealerEmail(value: string) {
    ...
    try {
      ...
      if (payload.login_required) {
        ...
        setLoginCodeSent(true);
        setShowContinuationLogin(true);
        setStatus(payload.message || "We found an existing secure funding review for this email. Enter the code we sent to continue.");
      }
    } catch {
  ...
  async function verifyDealerCode() {
    ...
    setStatus("");
    try {
      const payload = await call<IntakeResponse>("/public/funding-review/login/verify", {
        method: "POST",
  ...
  async function loadIntake(activeToken = token, fromResume = false) {
    const payload = await call<IntakeResponse>(`/public/funding-review/${encodeURIComponent(activeToken)}`);
  ...
  async function loadDealerSession(sessionToken: string) {
    const payload = await call<IntakeResponse>("/public/funding-review/session", {
      headers: { "X-Funding-Session": sessionToken },
    });
    persistDealerSession(payload);
    applyResponse(payload, payload.token ?? "", true);
    syncMessagesFromResponse(payload, true);
  ...
  async function sendChat(message?: string, updates?: Record<string, unknown>) {
    ...
    setStatus("");
    try {
      const payload = await call<IntakeResponse>(`/public/funding-review/${encodeURIComponent(token)}/chat`, {
        method: "POST",
  ...
  async function submitEntityStructure() {
    ...
    await sendChat("I clarified the borrower, vesting, and property ownership structure.", { entity_structure: cleaned });
  }
  ...
  async function exportIntelligencePdf() {
    ...
    const res = await fetch(`${apiBase}/api/v1/public/funding-review/intelligence.pdf?${params.toString()}`, {
      headers: dealerSessionToken ? { "X-Funding-Session": dealerSessionToken } : undefined,
    });
    ...
    link.href = url;
    link.download = `funding-intelligence-${response?.intake.business_name || response?.intake.full_name || "review"}.pdf`;
  ...
  async function bookCall(startsAt: string) {
    ...
    try {
      const payload = await call<IntakeResponse>(`/public/funding-review/${encodeURIComponent(token)}/book-call`, {
        method: "POST",
  ...
  async function openSecureRoom() {
    ...
    setRoomNotice("");
    try {
      const handoff = await call<SecureRoomHandoff>(`/public/funding-review/${encodeURIComponent(token)}/secure-room`, {
        method: "POST",
      });
  ...
  async function executeChatAction(action: IntakeChatAction) {
    ...
    try {
      const result = await call<IntakeChatActionResult>(
        `/public/funding-review/${encodeURIComponent(token)}/chat-actions/${action.id}`,
      ...
      if (action.action_type === "upload_own") {
        ...
        const download = await fetch(`${apiBase}${result.download_url}`);
        if (!download.ok) throw new Error(await responseMessage(download));
      ...
      for (const item of queuedFiles.filter((file) => file.status === "ready" || file.status === "error")) {
        ...
        try {
          ...
          const init = await call<{ file_id: string; upload_url: string; required_headers: Record<string, string> }>(
            `/public/funding-review/${encodeURIComponent(token)}/files/upload-init`,
            {
              method: "POST",
              body: JSON.stringify({
          ...
          await call(`/public/funding-review/${encodeURIComponent(token)}/files/complete`, {
            method: "POST",
  ...
  function logoutRoom() {
    ...
    if (currentSession) {
      call("/public/funding-review/logout", {
        method: "POST",
                ...
                <header style={workspaceHeaderMobile}>
                  <div style={brandGroupMobile}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <h1 style={workspaceTitleMobile}>Funding AI Underwriter</h1>
                    </div>
                  </div>
                  <button type="button" style={ghostButton} onClick={logoutRoom}>Logout</button>
                  <div style={mobileTabs} aria-label="Funding review room sections">
              ...
              <div style={compact ? workspaceGridMobile : activeWorkspace === "chat" ? workspaceGrid : workspaceGridSingle}>
                ...
                <section style={compact ? chatPanelModernMobile : chatPanelModern}>
                  ...
                  <div style={compact ? chatTopBarMobile : chatTopBar}>
                    <div>
                      <h2 style={sectionTitle}>Funding AI Underwriter</h2>
                  ...
                  <div style={compact ? messagesModernMobile : messagesModern}>
                    ...
                    {chat.map((line) => (
                      <Fragment key={line.id}>
                        {line.role === "operator" ? (
                        <div key={line.id} style={operatorBubble}>
                          <span style={operatorName}>{line.authorName?.trim() || c.underwriterFallbackName}</span>
                          {line.content}
                        </div>
                      ) : (
    ...
    <div style={stepOneFormCard}>
      ...
      <div style={stepOneFormGrid}>
        <SelectField
          ...
          value={contact.transaction_type}
          onChange={(value) => setContact({ ...contact, transaction_type: value })}
          options={["Purchase", "Refinance", "Cash-out refinance", "Portfolio review", "Not sure yet"]}
        />
        <SelectField
          label="Estimated credit"
          value={contact.estimated_credit_tier}
          onChange={(value) => setContact({ ...contact, estimated_credit_tier: value })}
          options={["Strong Credit", "Mid Credit", "Below Threshold", "Not sure yet"]}
        />
      </div>
      <div style={stepOneFormGrid}>
...
function EntityWidget({ entity, setEntity, busy, onSubmit }: { entity: EntityStructure; setEntity: (value: EntityStructure) => void; busy: boolean; onSubmit: () => void }) {
  return (
    <WidgetBox title="Borrower and property ownership" description="Clarify who owns the property, who will borrow, and how title or entity vesting should be reviewed.">
      <Field label="Borrowing entity / vesting" value={entity.primary_operating_entity} onChange={(value) => setEntity({ ...entity, primary_operating_entity: value })} placeholder="ABC Holdings LLC" />
      <Field label="Rent deposit account" value={entity.main_operating_bank_account} onChange={(value) => setEntity({ ...entity, main_operating_bank_account: value })} placeholder="Bank name and account purpose" />
      <TextAreaField label="Related entities" value={entity.related_entities} onChange={(value) => setEntity({ ...entity, related_entities: value })} placeholder="List related holding, property, borrower, or management entities" />
      <TextAreaField label="How ownership and cash flow connect" value={entity.relationship_explanation} onChange={(value) => setEntity({ ...entity, relationship_explanation: value })} placeholder="Explain leases, rent collection, management, and transfers if more than one entity is involved" />
      <button style={primaryWide} disabled={busy} onClick={onSubmit}>Save ownership structure</button>
    </WidgetBox>
  );
}
  ...
  return (
    <WidgetBox title="Upload baseline documents" description="Attach what you have now. You can keep chatting before every baseline item is uploaded.">
      <div style={baselineSummary}>
        ...
        <span>Upload property evidence only: rent support, purchase/payoff documents, PITIA support, value evidence, and entity/vesting items.</span>
      </div>
      <div style={chipWrap}>
...
function DealerSidebar({
  ...
  const tabs: Array<{ key: WorkspaceTab; label: string; detail: string }> = [
    ...
    { key: "files", label: "Files", detail: `${response.files.length} uploaded` },
  ...
  return (
    <aside style={dealerSidebar}>
      <div style={sidebarBrand}>
        ...
        <div>
          <strong>Qualified Commercial</strong>
          <span>Funding review room</span>
        </div>
      </div>

      <nav style={sidebarNav} aria-label="Funding review room">
        {tabs.map((tab) => {
      ...
      <div style={sidebarSection}>
        ...
        <div style={sidebarMiniCard}>
          ...
          <span>Property evidence, rent support, PITIA</span>
        </div>
      </div>
      ...
      <div style={sideSection}>
        ...
        <div style={chipWrap}>
          {missingDocs.length ? missingDocs.map((doc) => {
            ...
            return doc.requires_signature && setSigningDocId ? (
              <button
                key={doc.id}
                type="button"
    ...
    <section style={intelligenceCover}>
      ...
      <div style={intelligenceCoverCard}>
        ...
        <div style={intelligenceCoverNeeds}>
          ...
          <span>Rent support, PITIA, payoff/purchase evidence, and property value support.</span>
        </div>
      </div>
    </section>
  );
    ...
    <section style={intelligencePanel}>
      ...
      <div style={chartGrid}>
        ...
        <div style={chartCard}>
          ...
          <MiniBarChart series={model.monthlySeries} emptyLabel="Awaiting rent/PITIA evidence or monthly operating data." />
        </div>
        ...
        <div style={chartCard}>
          ...
          <MiniBarChart series={model.yearlySeries} emptyLabel="Awaiting property income and expense evidence." />
        </div>
      </div>
...
function SelectField({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) {
  return (
    <label style={fieldWrap}>
      ...
      <select style={select} value={value} onChange={(event) => onChange(event.target.value)}>
...
// A person from the desk. Incoming like the assistant, so it stays on the left,
// but carded and named against the assistant's plain text so the borrower can
// tell a human apart from the AI.
