...
// comes back rides along in `image_ids` when the note posts, which is the
// moment the image stops being an orphan and becomes part of something.
//
// Uploading before the note is written is deliberate. Someone pastes a
// screenshot and keeps typing; by the time they save, the picture is up.
//
// Mirrors QCRep's lib/inlineImages.ts — the two apps share no package, so keep
// the rules in step by hand.
...
type ApiCall = <T>(path: string, opts?: RequestInit) => Promise<T>;
...
export async function uploadInlineImage(
  ...
  const ticket = await apiCall<UploadTicket>("/inline-images/upload-init", {
    method: "POST",
    body: JSON.stringify({
  ...
  return apiCall<InlineImage>(`/inline-images/${ticket.image_id}/complete`, { method: "POST" });
}
