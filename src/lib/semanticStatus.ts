...
const STATUS_TONES: Record<SemanticStatusTone, Set<string>> = {
  ...
  processing: new Set(["confirmed", "follow_up", "queued", "uploading", "analyzing", "reviewing", "received_unverified", "stale", "processing", "pending", "in_progress", "qualifying", "in_underwriting", "needs_attention", "term_sheet_provided", "promising"]),
  approved: new Set(["active", "recommended", "indexed", "extracted", "prequalified", "verified", "waived", "not_applicable", "approved", "reviewed", "converted", "closed_won", "complete", "completed", "ready", "ready_for_funding", "funded", "sent", "delivered"]),
  denied: new Set(["error", "undelivered", "no_show", "not_qualified", "failed", "failed_review", "denied", "closed_lost", "rejected", "blocked"]),
...
export function semanticStatusClass(status: string | null | undefined): string {
  ...
  return `semantic-status semantic-status-${tone ?? "neutral"}`;
}

export function semanticChipTone(status: string | null | undefined): "mut" | "warn" | "ok" | "bad" {
  const tone = semanticStatusTone(status);
  return tone === "processing" ? "warn" : tone === "approved" ? "ok" : tone === "denied" ? "bad" : "mut";
}
